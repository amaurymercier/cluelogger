
export default data = [
   {
     _id: 1,
     title: "Momo n'est pas qui vous pensez...",
     text : "Aussi étonant que cela puisse paraitre, ce bon vieil Amaury est en réalité un boulanger. C'est chez Momo qu'on trouve les meilleures baguettes. Cherchez la mie en or, c'est sur lui que vous tomberez...",
     image_path : 'none',
     date : ''
   },
   {
     _id: 2,
     title: "La clef des champs se trouve chez Didier",
     text : "Eh oui ! Après une coupe du monde riche en émotions, notre meilleur sélectionneur est parti se reposer dans sa maison de campagne",
     image_path : 'none',
     date : ''
   },
   {
     _id: 3,
     title: "Deux dices valent mieux qu'in",
     text : "Voilà un drôle d'indice...",
     image_path : 'none',
     date : ''
   },
   {
     _id: 4,
     title: "Juju et Momo sont dans un bateau...",
     text : "Teub tombe à l'eau, qui reste-t-il ?",
     image_path : 'none',
     date : ''
   },
   {
     _id: 5,
     title: "J'ai du bon tabac dans ma tabatière",
     text : "J'ai du bon tabac mais tu n'en auras pas !",
     image_path : 'none',
     date : ''
   },
]
