...
import { StyleSheet, View, Text, ScrollView, Image, Button, TouchableOpacity, Share, Dimensions } from 'react-native'
...
class ClueDetail extends React.Component {
  ...
  _displayClueImage(Image_path) {
    ...
    const completeImagePath = 'https://olympyxt.herokuapp.com/rectangle_' + Image_path
  ...
  _displayClueFavIcon() {
    ...
    if (this.props.favoriteClues.findIndex(item => Clue._id === item._id) !== -1) {
      return (
...
// pour ajouter ou retirer une Clue des favorites
  ...
  _shareClue() {
    const Clue = this.state.Clue
    const shared_text = '🏆 Des nouvelles des Olympiades ! 🏆 \n \n' + Clue.description
  ...
  componentDidMount() {
    ...
    getClueWithIdFromApi(this.props.navigation.state.params.idClue).then(data => {
      // console.log(data)
      this.setState({
        Clue : data,
        isLoading: false
        ...
        <View style={styles.top_container}>
          <View style={styles.white_top}>
            <View style={styles.title_container}>
              <Text style={styles.title_text} numberOfLines={2}>{this.state.Clue.titre} </Text>
            </View>
            <View style={styles.icon_bar}>
              {this._displayClueFavIcon()}
              {this._displayShareIcon()}
              <Text style={styles.date_text}>{moment(new Date(this.state.Clue.date)).format('LL')}</Text>
            </View>
          </View>
        </View>

        <View style={styles.description_container}>
          <Text style={styles.description_text}>{this._adaptDatabaseText(this.state.Clue.texte)}</Text>
        </View>
